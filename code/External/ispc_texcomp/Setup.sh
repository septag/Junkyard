...
os_name=$(uname -s)
case "$os_name" in
    Linux*)  ISPC_DIST="${VERSION}/ispc_dist-linux-x86_64.zip";;
    Darwin*) ISPC_DIST="${VERSION}/ispc_dist-mac-arm64.zip";;
    *)       echo "Unsupported OS"; exit 1;;
esac

ZIPFILE="${ROOT}ispc.zip"

echo "https://github.com/septag/ISPCTextureCompressor/releases/download/${ISPC_DIST}"

if [[ ! -f "$ZIPFILE" ]]; then
    curl -fL \
        "https://github.com/septag/ISPCTextureCompressor/releases/download/${ISPC_DIST}" \
        -o "$ZIPFILE"
fi
